...
public class ControllerDescribeStreamCommand extends ControllerCommand {
    ...
    public void execute() throws Exception {
        ...
        StreamMetadataStore store = StreamStoreFactory.createZKStore(zkClient, ForkJoinPool.commonPool());
        // Output the configuration of this Stream.
        CompletableFuture<StreamConfiguration> streamConfig = store.getConfiguration(scope, stream, null, ForkJoinPool.commonPool());
        output("Stream configuration: " + streamConfig.join().toString());

        // Output the creation time for this Stream.
        output("Stream creation time: " + store.getCreationTime(scope, stream, null, ForkJoinPool.commonPool()).join());

        output("Stream state: " + store.getState(scope, stream, true, null, ForkJoinPool.commonPool()).join().toString());

        // Output the total number of segments for this Stream.
        Set<Long> segments = store.getAllSegmentIds(scope, stream, null, ForkJoinPool.commonPool()).join();
        output("Total number of Stream segments: ", segments.size());

        // Check if the Stream is sealed.
        output("Is Stream sealed? " +  store.isSealed(scope, stream, null, ForkJoinPool.commonPool()).join());

        // Output the active epoch for this Stream.
        EpochRecord epochRecord = store.getActiveEpoch(scope, stream, null, true, ForkJoinPool.commonPool()).join();
        output("Current Stream epoch: " + epochRecord.getEpoch() + ", creation time: " + epochRecord.getCreationTime());
